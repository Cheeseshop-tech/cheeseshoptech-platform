// Netlify Function: serve a tenant's LIVE market "signals" at runtime (no app rebuild). Reads the
// latest items array from Netlify Blobs (written by signals-publish.js, which
// scripts/publish-signals.mjs calls from the weekly signal-distillation automation). If Blobs is
// empty/unavailable, returns {items:null} so the front end falls back to the bundled sample in
// src/data/<tenant>/signals.json.
//
// Front end uses this when VITE_SIGNALS_BACKEND=function (see src/lib/signals.js).
// Mirrors attention-list.js / market-news.js exactly — same Blobs-or-fallback contract, same read guard.
...
const rawHandler = async (event, context) => {
  ...
  // Same read tier as live inventory/market-news/attention: any valid session/passcode may read.
...
export const handler = withMonitoring("signals-list", rawHandler);
