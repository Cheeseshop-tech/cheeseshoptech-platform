...
| F3 | Client content manager: catalog metadata editing → canonical data layer | **SHIPPED 2026-06-12.** Lightbox editing (code/title/description) for client-admin+, per-tenant edits overlay (`lib/catalog-edits.js`), export/import JSON feeding the price-list workflow; real backend replaces the seam later |
| F4 | Proposal engine v1 — **both tiers day one** (house pitches + client buyer proposals) | **SHIPPED 2026-06-12.** Builder (Manage tier, `?page=proposals`): pitch copy + story deck + SKU picker + class-of-trade; proposal travels in the share-link URL (`?page=proposal#p=…`, gated) and prices quote LIVE via pricing-core so links never go stale; print/PDF. Per-proposal revocable keys = v2 (needs backend) |
| F5 | Unified image pipeline + media storage automation | **Spec'd 2026-06-13: `IMAGE_PIPELINE_SPEC.md`.** R2 bucket created. Render layer already unified through one `cldImage` builder (shipped). Remaining = one sync job → one canonical `images.json` manifest per tenant → every surface reads it (retire buyer-catalog.json / sku.image / media-list drift) + fold in prewarm + masters→R2. |
