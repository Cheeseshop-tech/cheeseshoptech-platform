...
| Confirm `SENTRY_DSN` (functions half) is actually set in Netlify | Rick | P1 | **Open, attempted, deferred to Monday** — found it was set to a value that didn't match the project's current DSN; Rick edited it, but a local format-check script then rejected the pasted value as "invalid DSN," and whether the currently-saved value is fixed or reverted is unclear. Paused for today at Rick's call (2026-09-18: "temporary, just for today... fix this eventually tomorrow or maybe Monday") — functions read `process.env.SENTRY_DSN` at invocation time (not build time), so whenever it is actually corrected it takes effect without a redeploy. |
